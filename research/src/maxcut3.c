/*
 * maxcut3.c - Tabu search solver for Max-Cut (G-set style instances).
 * Extends maxcut2.c with two additional algorithms:
 *   - tsf: "ts" plus elite-consensus freezing across stagnation events.
 *   - icm: two interleaved "ts" replicas with isoenergetic cluster moves.
 *
 * Build:
 *   gcc -O2 -march=native -Wall -Wextra -std=c11 -o maxcut3 maxcut3.c -lm
 *
 * Usage:
 *   ./maxcut3 <instance_path> --algo {ts|ts2|tsf|icm} --seed <int> --time <seconds> --init {random|spectral}
 *   ./maxcut3 --selftest
 *
