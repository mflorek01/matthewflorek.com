...
The admin settings page is `/admin/ai` and requires the normal authenticated admin session. It can set or replace the API key, verify it, choose the model, enable or disable the assistant, and remove the stored credential. Keys are encrypted with AES-256-GCM, are never returned to the browser, and are represented only by their last four characters. The same page provides an upload-and-publish workflow for UTF-8 TXT, Markdown, CSV, and JSON knowledge documents. Uploads are private drafts until **Publish for AI** is selected; unpublishing or archiving removes them from retrieval without deleting the stored file. The protected document inspection endpoint is `GET /api/ai/admin/knowledge` with the `X-AI-Admin-Token` header. It never returns document content or private records.
